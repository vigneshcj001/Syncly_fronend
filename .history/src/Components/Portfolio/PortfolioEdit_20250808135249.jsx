...
const EditSection = ({
  ...
  const handleEditClick = (item) => {
    ...
    fields.forEach(field => {
      if (field.type === 'date' && item[field.name]) {
        initialFormData[field.name] = new Date(item[field.name]).toISOString().split('T')[0];
      } else {
  ...
  return (
    <section className="mt-8">
      ...
      <form onSubmit={handleSubmit} className="p-4 border rounded-lg bg-gray-50 dark:bg-gray-800 space-y-4 mb-6">
        <h3 className="text-xl font-bold">{editingId ? `Editing ${title.slice(0, -1)}` : `Add New ${title.slice(0, -1)}`}</h3>
      ...
      <ul className="space-y-3">
        {items && items.map((item) => (
          <li key={item._id} className="p-3 border rounded-lg flex justify-between items-center shadow-sm">
            <div>
              <p className="font-bold">{item.title || item.role || item.degree || item.name}</p>
              <p className="text-sm text-gray-600">{item.description || item.institution || item.company || item.issuingOrganization}</p>
            </div>
            <div className="flex gap-2">
              <button onClick={() => handleEditClick(item)} className="px-3 py-1 border rounded hover:bg-gray-100">
                Edit
              </button>
              <button onClick={() => handleDeleteClick(item._id)} className="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600">
                Delete
              </button>
            </div>
          </li>
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <EditSection
        ...
        api={{ create: createProject, update: updateProject, delete: deleteProject }}
      ...
      <EditSection
        ...
        api={{ create: createExperience, update: updateExperience, delete: deleteExperience }}
        onUpdate={loadAllData}
      />
      ...
      <EditSection
        ...
        api={{ create: createEducation, update: updateEducation, delete: deleteEducation }}
        onUpdate={loadAllData}
      />
      ...
      <EditSection
        ...
        fields={[
          ...
          { name: "issuingOrganization", placeholder: "Issuing Organization", required: true },
          { name: "issueDate", placeholder: "Issue Date", type: 'date', required: true },
          { name: "credentialURL", placeholder: "Credential URL" },
        ]}
        api={{ create: createCertification, update: updateCertification, delete: deleteCertification }}
        onUpdate={loadAllData}
      />
    </div>
  );
